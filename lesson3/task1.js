...
// где проверка? => Проверьте работу скрипта при a, равном 1, 0, -3.
...
// где проверка? => Проверьте работу скрипта при a, равном 'тест', 3.
...
if (test == true){
    // тут можно использвать короткую запись if(test)
    console.log("Correct!");
}
else {
    console.log("Incorrect!");
...
// где проверка:  Проверьте работу скрипта при test, равном true, false
...
// массив arr = ["пн","вт","ср","чт","пт","сб","вс"]; можно было обьявить 1 раз а потом исопльзовть переменную в if/else && switch
